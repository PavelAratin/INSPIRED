...
export const renderProducts = async (title, params) => {
  ...
  if (Object.hasOwn(data.data, 'totalCount')) {
    createElement('sup', {
      className: 'goods__title',
      innerHTML: `&nbsp(${data.data.totalCount})`
    }, {
      parent: titleElem
    })
  ...
  const newProductsCards = goods.map(product => {
    ...
    const newProducsBody = createElement('div', {
      ...
      <ul class="list-actions favorite">
        <li class="list-actions__item">
          <button data-id=${product.id} class="favorite__button ${favoriteList.includes(product.id) ? 'favorite__active' : ''}">
